...
class FeatureUtils:
    ...
    @staticmethod
    def handle_missing_values(data: pd.DataFrame, method: str = "forward_fill", limit: Optional[int] = None) -> pd.DataFrame:
    ...
    @staticmethod
    def remove_outliers(data: pd.DataFrame, method: str = "iqr", threshold: float = 1.5) -> pd.DataFrame:
    ...
    @staticmethod
    def scale_features(data: pd.DataFrame, method: str = "standard", columns: Optional[List[str]] = None) -> pd.DataFrame:
    ...
    @staticmethod
    def create_lag_features(data: pd.Series, lags: List[int], name_prefix: str = "lag") -> pd.DataFrame:
    ...
    def create_rolling_features(
        data: pd.Series, windows: List[int], functions: List[str] = ["mean", "std", "min", "max"], name_prefix: str = "rolling"
    ) -> pd.DataFrame:
    ...
    def create_interaction_features(
        data: pd.DataFrame, feature_pairs: List[tuple], operations: List[str] = ["multiply", "divide", "add", "subtract"]
    ) -> pd.DataFrame:
    ...
    @staticmethod
    def select_features_by_correlation(data: pd.DataFrame, target: pd.Series, threshold: float = 0.1) -> List[str]:
