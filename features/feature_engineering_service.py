...
class FeatureEngineeringService:
    ...
    def engineer_features(
        self, df: pd.DataFrame, feature_types: List[str] = None, target_column: str = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            result_df = df.copy()
            feature_metadata = {"original_features": list(df.columns), "feature_types": feature_types, "engineering_steps": []}
                ...
                feature_metadata["engineering_steps"].append(
                    {
                        "step": "time_features",
                        "duration_seconds": step_duration,
                        "features_added": len(result_df.columns)
                        - len(df.columns)
                        - feature_metadata["engineering_steps"][-1]["features_added"]
                        if feature_metadata["engineering_steps"]
                        else 0,
                    }
                )
                ...
                feature_metadata["engineering_steps"].append(
                    {
                        ...
                        "features_added": len(result_df.columns)
                        - len(df.columns)
                        - sum(step["features_added"] for step in feature_metadata["engineering_steps"]),
                    }
                )
            ...
            if "automated" in feature_types and self.config["automated_feature_engineering"]:
                ...
                step_start = datetime.now()
                automated_result = self.automated_fe.engineer_features_automated(result_df, target_column=target_column)
                step_duration = (datetime.now() - step_start).total_seconds()
            ...
            return {"success": True, "data": result_df, "metadata": feature_metadata}

        except Exception as e:
            logger.error(f"Error in feature engineering: {str(e)}")
            return {"success": False, "error": str(e), "data": df}

    def _select_features(self, df: pd.DataFrame, target_column: str) -> tuple[pd.DataFrame, Dict[str, Any]]:
        """
    ...
    def engineer_features_for_symbols(
        self, data_dict: Dict[str, pd.DataFrame], feature_types: List[str] = None
    ) -> Dict[str, Any]:
        """
