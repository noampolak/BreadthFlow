...
from pyspark.sql.functions import col, count, date_format, dayofmonth, dense_rank, expr, lag, lead, month, rank, row_number
...
class ADFeatures:
    ...
    def _calculate_ad_line(self, daily_ad: DataFrame) -> DataFrame:
        ...
        ad_diff = daily_ad.withColumn("ad_difference", col("advancing_stocks") - col("declining_stocks"))

        # Calculate cumulative A/D line
        window_spec = Window.orderBy("date")
        ad_line = ad_diff.withColumn("ad_line", spark_sum("ad_difference").over(window_spec))
    ...
    def _calculate_ad_momentum(self, ad_line: DataFrame) -> DataFrame:
        ...
        ad_momentum = ad_line.withColumn(
            "ad_momentum", (col("ad_line") - lag("ad_line", self.lookback_period).over(window_spec)) / self.lookback_period
        )
    ...
    def calculate_ad_features_from_delta(
        ...
        if output_path:
            ...
            write_delta(df=ad_features, path=output_path, partition_cols=["year", "month"], mode="append")
    ...
    def detect_ad_signals(self, ad_features: DataFrame) -> DataFrame:
        ...
        signals = (
            ad_features.withColumn(
                "ad_bullish_signal", (col("ad_issues_ratio") > 2.0) & (col("ad_volume_ratio") > 1.5) & (col("ad_momentum") > 0)
            )
            .withColumn(
                "ad_bearish_signal",
            ...
            .withColumn(
                "ad_thrust_signal", (col("ad_issues_ratio") > 3.0) & (col("ad_volume_ratio") > 2.0) & (col("ad_momentum") > 5)
            )
            .withColumn("ad_divergence_signal", (col("ad_issues_ratio") > 1.5) & (col("ad_volume_ratio") < 0.8))
        )

        return signals

    def calculate_ad_rolling_metrics(self, ad_features: DataFrame, window_days: int = 20) -> DataFrame:
        """
...
if __name__ == "__main__":
    ...
    spark = (
        SparkSession.builder.appName("ADFeaturesTest")
