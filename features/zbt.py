...
class ZweigBreadthThrust:
    ...
    def _calculate_daily_issues(self, df: DataFrame) -> DataFrame:
        ...
        daily_issues = daily_issues.withColumn(
            "total_issues",
            col("advancing_issues") + col("declining_issues") + col("unchanged_issues")
        ).withColumn(
            "advancing_percentage",
            when(col("total_issues") > 0, col("advancing_issues") / col("total_issues")).otherwise(0.0)
        )
    ...
    def _calculate_zbt_percentage(self, daily_issues: DataFrame) -> DataFrame:
        ...
        zbt_percentage = daily_issues.withColumn(
            "zbt_advancing_sum",
            spark_sum("advancing_issues").over(window_spec)
        ).withColumn(
            "zbt_total_sum",
            spark_sum("total_issues").over(window_spec)
        ).withColumn(
    ...
    def _detect_zbt_signals(self, zbt_percentage: DataFrame) -> DataFrame:
        ...
        zbt_signals = zbt_percentage.withColumn(
            "above_threshold",
            col("zbt_10day_percentage") >= self.zbt_threshold
        )
        ...
        zbt_signals = zbt_signals.withColumn(
            ...
            "zbt_signal",
            (col("above_threshold")) & (col("recent_above_threshold") == 0)
        )
    ...
    def _calculate_zbt_metrics(self, zbt_signals: DataFrame) -> DataFrame:
        ...
        zbt_metrics = zbt_signals.withColumn(
            "zbt_strength",
            when(col("zbt_signal"), col("zbt_10day_percentage") - self.zbt_threshold).otherwise(0.0)
        )
        
        # Calculate ZBT momentum (rate of change)
        zbt_metrics = zbt_metrics.withColumn(
            "zbt_momentum",
            col("zbt_10day_percentage") - lag("zbt_10day_percentage", 5).over(window_spec)
        )
        ...
        zbt_metrics = zbt_metrics.withColumn(
            "zbt_confidence",
            when(col("zbt_signal"),
                 (col("zbt_strength") * 0.4 + 
                  col("zbt_momentum") * 0.3 + 
                  col("zbt_rarity_score") * 0.3)
            ).otherwise(0.0)
        )
    ...
    def detect_zbt_signals(self, zbt_result: DataFrame) -> DataFrame:
        ...
        signals = zbt_result.withColumn(
            "zbt_bullish_signal",
            (col("zbt_signal")) & 
            (col("zbt_confidence") > 0.7) & 
            (col("zbt_strength") > 0.05)
        ).withColumn(
            "zbt_strong_signal",
            (col("zbt_signal")) & 
            ...
            "zbt_momentum_signal",
            (col("zbt_10day_percentage") > 0.7) & 
            (col("zbt_momentum") > 0.05) & 
            (col("zbt_rarity_score") > 1.5)
        ).withColumn(
            "zbt_breakout_signal",
            (col("zbt_signal")) & 
            (col("zbt_10day_percentage") > 0.75) & 
            (col("zbt_momentum") > 0.1)
        )
        
        return signals
    
    def calculate_zbt_rolling_metrics(
        self, 
        zbt_result: DataFrame, 
        window_days: int = 20
    ) -> DataFrame:
        """
        ...
        rolling_metrics = zbt_result.withColumn(
            ...
            "zbt_trend",
            expr(f"avg(zbt_10day_percentage) over (order by date rows between {window_days} preceding and current row)") - 
            expr(f"avg(zbt_10day_percentage) over (order by date rows between {window_days*2} preceding and {window_days} preceding)")
        )
