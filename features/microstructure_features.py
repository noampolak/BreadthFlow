...
class MicrostructureFeatures:
    ...
    def _add_volume_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            for period in [5, 10, 20]:
                ...
                df[f'volume_zscore_{period}'] = (df['volume'] - df[f'volume_sma_{period}']) / df['volume'].rolling(window=period).std()
    ...
    def _add_price_volume_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df['price_vs_vwap_zscore'] = (df['price_vs_vwap'] - df['price_vs_vwap'].rolling(window=20).mean()) / df['price_vs_vwap'].rolling(window=20).std()
            ...
            df['volume_weighted_return'] = (df['close'].pct_change() * df['volume']).rolling(window=20).sum() / df['volume'].rolling(window=20).sum()
    ...
    def _add_order_flow_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df['spread_proxy_zscore'] = (df['spread_proxy'] - df['spread_proxy_ma']) / df['spread_proxy'].rolling(window=20).std()
    ...
    def _add_trading_intensity_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Volume persistence
            df['volume_persistence'] = df['volume_burst'].rolling(window=10).apply(lambda x: self._longest_consecutive(x), raw=True)
    ...
    def _add_market_quality_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df['price_impact_measure'] = abs(df['close'].pct_change()) / (df['volume'] / df['volume'].rolling(window=20).mean())
        ...
        try:
            summary = {
                ...
                "price_volume_relationship": {
                    "price_volume_correlation": df['close'].corr(df['volume']) if 'close' in df.columns and 'volume' in df.columns else 0,
