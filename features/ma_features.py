...
from pyspark.sql.functions import (
    col, when, sum as spark_sum, avg, lag, lead, 
    window, expr, row_number, rank, dense_rank,
    date_format, year, month, dayofmonth, count
)
...
class MAFeatures:
    ...
    def _calculate_moving_averages(self, df: DataFrame) -> DataFrame:
        ...
        # Calculate moving averages
        ma_features = df.withColumn(
            "ma_short",
            avg("close").over(window_spec_short)
        ).withColumn(
            "ma_long",
            avg("close").over(window_spec_long)
        )
        
        # Calculate price vs MA relationships
        ma_features = ma_features.withColumn(
            "price_vs_ma_short",
            (col("close") - col("ma_short")) / col("ma_short")
        ).withColumn(
            "price_vs_ma_long",
            (col("close") - col("ma_long")) / col("ma_long")
        )
    ...
    def _calculate_crossover_signals(self, ma_features: DataFrame) -> DataFrame:
        ...
        crossover_signals = ma_features.withColumn(
            "ma_short_lag",
            lag("ma_short", 1).over(window_spec)
        ).withColumn(
            "ma_long_lag",
            lag("ma_long", 1).over(window_spec)
        )
        
        # Detect crossovers
        crossover_signals = crossover_signals.withColumn(
            "golden_cross",
            (col("ma_short") > col("ma_long")) & 
            (col("ma_short_lag") <= col("ma_long_lag"))
        ).withColumn(
            "death_cross",
            (col("ma_short") < col("ma_long")) & 
            (col("ma_short_lag") >= col("ma_long_lag"))
            ...
            "ma_cross_signal",
            when(col("golden_cross"), "golden_cross")
            .when(col("death_cross"), "death_cross")
            .otherwise("none")
        )
    ...
    def _calculate_momentum_features(self, crossover_signals: DataFrame) -> DataFrame:
        ...
        momentum_features = crossover_signals.withColumn(
            "ma_short_slope",
            (col("ma_short") - lag("ma_short", 5).over(window_spec)) / 5
        ).withColumn(
            "ma_long_slope",
            (col("ma_long") - lag("ma_long", 10).over(window_spec)) / 10
        )
        
        # Calculate overall MA momentum
        momentum_features = momentum_features.withColumn(
            "ma_momentum",
            (col("ma_short_slope") + col("ma_long_slope")) / 2
        )
        
        # Calculate trend strength
        momentum_features = momentum_features.withColumn(
            "ma_trend_strength",
            abs(col("ma_short") - col("ma_long")) / col("ma_long")
        )
    ...
    def detect_ma_signals(self, ma_features: DataFrame) -> DataFrame:
        ...
        signals = ma_features.withColumn(
            "ma_bullish_signal",
            (col("golden_cross")) & 
            (col("ma_momentum") > 0) & 
            (col("price_vs_ma_short") > 0.02)
        ).withColumn(
            "ma_bearish_signal",
            (col("death_cross")) & 
            (col("ma_momentum") < 0) & 
            ...
            (col("ma_short") > col("ma_long")) & 
            (col("ma_momentum") > 0.01) & 
            (col("ma_trend_strength") > 0.05)
        ).withColumn(
            "ma_reversal_signal",
            (col("ma_cross_signal") != "none") & 
            (abs(col("ma_momentum")) > 0.02)
        )
        
        return signals
    
    def calculate_ma_rolling_metrics(
        self, 
        ...
        rolling_metrics = ma_features.withColumn(
            "rolling_ma_momentum",
            expr(f"avg(ma_momentum) over (partition by symbol order by date rows between {window_days} preceding and current row)")
        ).withColumn(
            "rolling_ma_trend_strength",
            expr(f"avg(ma_trend_strength) over (partition by symbol order by date rows between {window_days} preceding and current row)")
        ).withColumn(
            "rolling_price_vs_ma_short",
            expr(f"avg(price_vs_ma_short) over (partition by symbol order by date rows between {window_days} preceding and current row)")
        ).withColumn(
            "rolling_price_vs_ma_long",
            expr(f"avg(price_vs_ma_long) over (partition by symbol order by date rows between {window_days} preceding and current row)")
        )
