...
class MarketMicrostructure:
    ...
    @staticmethod
    def calculate_volume_indicators(volume: Union[pd.Series, np.ndarray],
        ...
        obv = np.where(close.diff() > 0, volume, 
                      np.where(close.diff() < 0, -volume, 0)).cumsum()
    ...
    @staticmethod
    def calculate_price_impact(high: Union[pd.Series, np.ndarray],
                             low: Union[pd.Series, np.ndarray],
                             close: Union[pd.Series, np.ndarray],
                             volume: Union[pd.Series, np.ndarray]) -> Dict[str, pd.Series]:
    ...
    @staticmethod
    def calculate_volatility_indicators(close: Union[pd.Series, np.ndarray],
                                      period: int = 20) -> Dict[str, pd.Series]:
    ...
    def get_all_microstructure(ohlcv_data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Volume indicators
        volume_data = MarketMicrostructure.calculate_volume_indicators(
        ...
        volatility_data = MarketMicrostructure.calculate_volatility_indicators(
            ohlcv_data['Close']
        )
