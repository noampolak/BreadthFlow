...
class DataValidationService:
            ...
            for col_name, expected_type in expected_schema.items():
                if col_name in actual_columns:
                    ...
                    if expected_type not in actual_type.lower():
                        type_mismatches.append({"column": col_name, "expected": expected_type, "actual": actual_type})
    ...
    def validate_data_quality(self, df: DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if total_records == 0:
                return {"is_valid": False, "error": "No data to validate", "total_records": 0}
            ...
            is_valid = (
                ...
                and missing_stats["missing_ratio"] <= self.quality_thresholds["max_missing_ratio"]
                and price_stats["is_valid"]
                and volume_stats["is_valid"]
            )
    ...
    def detect_data_drift(self, current_df: DataFrame, reference_df: DataFrame, columns: List[str] = None) -> Dict[str, Any]:
        """
    ...
    def validate_training_data(self, df: DataFrame, target_column: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            is_valid = (
                ...
                and unique_symbols >= 10  # Minimum records for training
                and date_span >= 30  # Minimum symbols
                and future_data == 0  # Minimum time span
                and (not target_column or target_validation.get("is_valid", True))  # No future data
            )
    ...
    def _calculate_column_stats(self, df: DataFrame, col_name: str) -> Dict[str, Any]:
        ...
        try:
            stats = df.agg(
                {col_name: "count", col_name: "mean", col_name: "stddev", col_name: "min", col_name: "max"}
            ).collect()[0]
    ...
    def _calculate_drift_metrics(self, current_stats: Dict, reference_stats: Dict) -> Dict[str, float]:
        ...
        try:
            # Mean shift
            mean_shift = (
                abs(current_stats["mean"] - reference_stats["mean"]) / reference_stats["mean"]
                if reference_stats["mean"] != 0
                else 0
            )

            # Standard deviation change
            std_change = (
                abs(current_stats["stddev"] - reference_stats["stddev"]) / reference_stats["stddev"]
                if reference_stats["stddev"] != 0
                else 0
            )
            ...
            return {"mean_shift": mean_shift, "std_change": std_change, "ks_statistic": ks_statistic}

        except Exception as e:
            logger.error(f"Error calculating drift metrics: {str(e)}")
            return {"error": str(e)}

    def _calculate_quality_score(self, missing_stats: Dict, price_stats: Dict, volume_stats: Dict) -> float:
    ...
    def _generate_training_recommendations(
        self, total_records: int, unique_symbols: int, date_span: int, future_data: int
    ) -> List[str]:
