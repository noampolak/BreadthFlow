...
app = FastAPI(
    title="Data Pipeline API", description="API for data ingestion, validation, and storage for ML training", version="1.0.0"
)
...
async def startup_event():
    ...
    try:
        ...
        spark_session = (
            SparkSession.builder.appName("BreadthFlowDataPipeline")
...
async def root():
    """Root endpoint."""
    return {"service": "Data Pipeline API", "version": "1.0.0", "status": "running"}
...
async def get_symbol_lists():
    ...
    try:
        ...
        return {"symbol_lists": list(symbol_lists.keys()), "details": symbol_lists}
    except Exception as e:
...
async def ingest_data(request: IngestRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        result = data_ingestion.ingest_symbol_list(request.symbol_list, request.start_date, request.end_date)
        ...
        if request.validate_data:
            ...
            validation_result = {"validation_requested": True, "validation_status": "pending"}
        else:
...
async def get_storage_summary():
    ...
    try:
        ...
        return {"storage_summary": summary, "timestamp": datetime.now().isoformat()}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_objects(prefix: str = "", limit: int = 100):
    ...
    try:
        ...
        return {"objects": objects[:limit], "total_count": len(objects), "prefix": prefix}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def cleanup_old_data(days_to_keep: int = 30, data_type: str = None):
    """Clean up old data."""
    try:
        result = minio_storage.cleanup_old_data(days_to_keep=days_to_keep, data_type=data_type)
        return {"cleanup_result": result, "timestamp": datetime.now().isoformat()}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
