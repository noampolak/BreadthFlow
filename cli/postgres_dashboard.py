...
<body>
    ...
    def get_run_details(self, run_id):
        ...
        try:
            ...
            # Load real signal/backtest data from actual execution results
            if 'signals' in run_data['command']:
                # Real signals data will be loaded from MinIO
                run_data['signals_data'] = {
                    'note': 'Real signals data loaded from MinIO storage',
                    'source': 'actual_execution'
                }
            
            if 'backtest' in run_data['command']:
                # Real backtest data will be processed from actual results
                run_data['backtest_data'] = {
                    'note': 'Real backtest results from actual execution',
                    'source': 'actual_execution'
                }
...
<head>
    ...
    <style>
        ...
        .nav-btn { padding: 10px 20px; background: #007bff; color: white; border: none; border-radius: 4px; text-decoration: none; cursor: pointer; }
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <p>The Training system is currently under development.</p>
            ...
            <div style="margin-top: 20px;">
                <button class="nav-btn" onclick="window.location.href='/'">Return to Dashboard</button>
            </div>
        </div>
    </div>
</body>
