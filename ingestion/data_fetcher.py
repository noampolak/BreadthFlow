...
class DataFetcher:
    ...
    def fetch_symbol_data(
        ...
        try:
            ...
            schema_columns = ['symbol', 'date', 'open', 'high', 'low', 'close', 'volume', 'adj_close', 'fetched_at', 'data_quality_score']
    ...
    def fetch_multiple_symbols(
        ...
        result_df = result_df.withColumn("year", year(col("date"))) \
        ...
        logger.info(f"📊 Breakdown: mapPartitions={map_duration:.2f}s, DataFrame={df_duration:.2f}s, partitioning={partition_duration:.2f}s")
    ...
    def _validate_data_quality(self, df: pd.DataFrame) -> bool:
        ...
        return (quality_score >= self.min_data_completeness and 
                completeness >= self.min_data_completeness and 
                len(df) >= min_data_points)
