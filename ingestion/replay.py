...
class ReplayManager:
    ...
    def load_historical_data(
        ...
        # Add replay metadata
        df = df.withColumn("replay_timestamp", col("date")).withColumn("original_date", col("date"))
    ...
    def replay_data(self, df: DataFrame, config: ReplayConfig) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for batch_start in range(0, len(pandas_df), config.batch_size):
                ...
                if self.current_batch % 10 == 0:
                    ...
                    logger.info(
                        f"Progress: {progress:.1f}% ({self.processed_records}/{self.total_records}) - {elapsed:.1f}s elapsed"
                    )
    ...
    def _process_batch(self, batch_df: pd.DataFrame, config: ReplayConfig) -> Dict[str, Any]:
        """
        ...
        for _, row in batch_df.iterrows():
            try:
                ...
                # Send to Kafka
                future = self.kafka_producer.send(topic=config.topic_name, key=row["symbol"], value=message)
    ...
    def _prepare_message(self, row: pd.Series, config: ReplayConfig) -> Dict[str, Any]:
        ...
        if config.include_metadata:
            message["metadata"] = {
                ...
                if "data_quality_score" in row and pd.notna(row["data_quality_score"])
                else None,
                "fetched_at": row["fetched_at"].isoformat()
                if "fetched_at" in row and hasattr(row["fetched_at"], "isoformat")
                else None,
                "replay_batch": self.current_batch,
                "replay_config": {"speed_multiplier": config.speed_multiplier, "topic_name": config.topic_name},
            }
    ...
    def create_replay_stream(self, df: DataFrame, config: ReplayConfig) -> Iterator[Dict[str, Any]]:
        """
...
if __name__ == "__main__":
    ...
    spark = (
        SparkSession.builder.appName("ReplayManagerTest")
    ...
    try:
        ...
        df = replay_manager.load_historical_data(
            table_path="data/ohlcv", start_date=config.start_date, end_date=config.end_date, symbols=config.symbols
        )
