...
class AutoMLManager:
    ...
    def __init__(
        ...
        include_frameworks: List[str] = None
    ):
        """
        ...
        if "auto_sklearn" in include_frameworks:
            self.frameworks["auto_sklearn"] = AutoSklearnIntegration(
                time_limit=time_limit,
                memory_limit=memory_limit
            )
        
        if "tpot" in include_frameworks:
            self.frameworks["tpot"] = TPOTIntegration(
                generations=5,
                population_size=20
            )
        
        if "h2o" in include_frameworks:
            self.frameworks["h2o"] = H2OIntegration(
                max_models=20,
