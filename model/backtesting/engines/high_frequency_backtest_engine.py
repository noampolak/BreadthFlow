...
class HighFrequencyBacktestEngine(BaseBacktestEngine):
    ...
    def add_tick_data(self, symbol: str, timestamp: datetime, bid: float, ask: float, 
                     volume: float, trade_price: Optional[float] = None):
                        ...
                        if trade and trade.quantity > 0:
                            ...
                            if self.risk_manager.validate_trade(trade, self.current_portfolio, self.config):
                                ...
                                if executed_trade:
                                    ...
                                    logger.info(f"HFT Trade executed: {executed_trade.symbol} {executed_trade.trade_type.value} {executed_trade.quantity} at ${executed_trade.price:.4f}")
                            else:
    ...
    def get_hft_metrics(self) -> Dict[str, Any]:
        ...
        latency_metrics = {
            ...
            'latency_std_ms': np.std([t.execution_latency for t in self.trades_history if hasattr(t, 'execution_latency')]) * 1000
        }
