...
class StandardBacktestEngine(BaseBacktestEngine):
                    ...
                    if trade and trade.quantity > 0:
                        ...
                        if self.risk_manager.validate_trade(trade, self.current_portfolio, self.config):
                            ...
                            if executed_trade:
                                ...
                                logger.info(
                                    f"Trade executed: {executed_trade.symbol} {executed_trade.trade_type.value} {executed_trade.quantity}"
                                )
                        else:
    ...
    def _apply_position_sizing(self, trade: TradeRecord) -> Optional[TradeRecord]:
        ...
        if portfolio_value > 0:
            max_concentration = (
                self.config.max_position_concentration if hasattr(self.config, "max_position_concentration") else 0.1
            )
    ...
    def _calculate_daily_returns(self):
        ...
        if previous_value > 0:
            ...
            self.daily_returns.append(
                {"date": self.equity_curve[-1]["timestamp"].date(), "return": daily_return, "portfolio_value": current_value}
            )
    ...
    def _calculate_monthly_returns(self) -> List[Dict[str, Any]]:
        ...
        for month, returns in monthly_returns.items():
            ...
            result.append({"month": month, "return": cumulative_return, "days": len(returns)})
