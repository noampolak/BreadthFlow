...
class BacktestEngineInterface(ABC):
    ...
    @abstractmethod
    def run_backtest(
        self, config: BacktestConfig, price_data: Dict[str, pd.DataFrame], signal_data: pd.DataFrame
    ) -> Dict[str, Any]:
    ...
    def validate_data(self, price_data: Dict[str, pd.DataFrame], signal_data: pd.DataFrame) -> bool:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {"total_backtests": 0, "successful_backtests": 0, "failed_backtests": 0, "average_execution_time": 0.0}

    def get_backtest_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        """Get summary of backtest results"""
        if not results:
            return {"total_trades": 0, "total_return": 0.0, "sharpe_ratio": 0.0, "max_drawdown": 0.0, "win_rate": 0.0}

        trades = results.get("trades", [])
        portfolio_history = results.get("portfolio_history", [])

        summary = {"total_trades": len(trades), "total_return": 0.0, "sharpe_ratio": 0.0, "max_drawdown": 0.0, "win_rate": 0.0}
    ...
    def generate_trade_signals(self, signal_data: pd.DataFrame, config: BacktestConfig) -> List[Dict[str, Any]]:
    ...
    def calculate_position_size(self, signal: Dict[str, Any], portfolio_value: float, config: BacktestConfig) -> float:
