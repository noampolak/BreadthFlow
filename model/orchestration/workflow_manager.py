...
class WorkflowManager:
    ...
    async def _execute_workflow_task(self, execution_id: str):
        ...
        try:
            ...
            failed_steps = [step_id for step_id, status in execution.steps.items() if status == StepStatus.FAILED]
    ...
    async def _execute_steps(self, execution: WorkflowExecution, workflow: WorkflowDefinition):
        ...
        while len(completed_steps) < len(workflow.steps):
            ...
            ready_steps = []
            for step in workflow.steps:
                if step.step_id not in completed_steps and step_dependencies[step.step_id].issubset(completed_steps):
    ...
    async def _execute_step(self, execution: WorkflowExecution, step: WorkflowStep) -> Any:
        ...
        try:
            ...
            result = await asyncio.wait_for(self._execute_step_with_retry(step), timeout=step.timeout_seconds)
        ...
        for attempt in range(step.retry_attempts):
            try:
                ...
                if attempt < step.retry_attempts - 1:
                    self.logger.warning(
                        f"Step {step.step_id} attempt {attempt + 1} failed, retrying in {step.retry_delay_seconds} seconds: {e}"
                    )
