...
class SystemMonitor:
        ...
        while not self.stop_event.is_set():
            try:
                ...
                self.error_handler.record_error(
                    error=e, context={"monitoring_loop": True}, component="SystemMonitor", operation="monitoring_loop"
                )
    ...
    def _collect_metrics(self):
        ...
        if not PSUTIL_AVAILABLE:
            ...
            metrics = [
                Metric("system_monitoring", 1.0, MetricType.GAUGE, timestamp, description="System monitoring active"),
                Metric("psutil_available", 0.0, MetricType.GAUGE, timestamp, description="psutil library available"),
            ]
        else:
            # System metrics
            ...
            # Create metrics
            metrics = [
                Metric("cpu_usage_percent", cpu_percent, MetricType.GAUGE, timestamp, description="CPU usage percentage"),
                Metric(
                    "memory_usage_percent", memory.percent, MetricType.GAUGE, timestamp, description="Memory usage percentage"
                ),
                Metric(
                    "memory_available_gb",
                ...
                Metric("disk_usage_percent", disk.percent, MetricType.GAUGE, timestamp, description="Disk usage percentage"),
                Metric(
                    "disk_free_gb", disk.free / (1024**3), MetricType.GAUGE, timestamp, description="Free disk space in GB"
                ),
                Metric(
                    "process_cpu_percent", process_cpu, MetricType.GAUGE, timestamp, description="Process CPU usage percentage"
                ),
                Metric(
                    "process_memory_mb",
                ...
                Metric(
                    "network_bytes_sent", network.bytes_sent, MetricType.COUNTER, timestamp, description="Network bytes sent"
                ),
                Metric(
                    "network_bytes_recv",
                    network.bytes_recv,
                ...
                Metric("psutil_available", 1.0, MetricType.GAUGE, timestamp, description="psutil library available"),
            ]
    ...
    def _run_health_checks(self):
        ...
        for name, check_function in self.health_checks.items():
            try:
                ...
                health_checks.append(
                    HealthCheck(
                        name=name, status=HealthStatus.CRITICAL, message=f"Health check failed: {str(e)}", timestamp=timestamp
                    )
                )
    ...
    def _check_error_rate(self) -> HealthCheck:
        ...
        try:
            ...
            recent_errors = [
                error for error in self.error_handler.get_errors() if error.timestamp > datetime.now() - timedelta(hours=1)
            ]
    ...
    def _check_component_registry(self) -> HealthCheck:
        ...
        try:
            ...
            return HealthCheck(
                ...
                timestamp=datetime.now(),
                details={"components_registered": 0, "registry_accessible": True},  # Would be actual count
            )
    ...
    def _update_system_status(self):
        ...
        latest_checks = [check for check in self.health_history if check.timestamp > datetime.now() - timedelta(minutes=5)]
        ...
        # Get latest metrics
        latest_metrics = [
            metric for metric in self.metrics_history if metric.timestamp > datetime.now() - timedelta(minutes=5)
        ]
    ...
    def _check_alerts(self):
        ...
        self.alerts = [
            alert for alert in self.alerts if "Alert:" not in alert or datetime.now() - timedelta(hours=1) < datetime.now()
        ]
    ...
    def get_system_status(self) -> Optional[SystemStatus]:
        ...
        if self.current_status is None:
            ...
            if self.current_status is None:
                self.current_status = SystemStatus(
                    timestamp=datetime.now(), overall_health=HealthStatus.HEALTHY, health_checks=[], metrics=[], alerts=[]
                )
        return self.current_status

    def get_metrics(self, metric_name: Optional[str] = None, time_range: Optional[timedelta] = None) -> List[Metric]:
    ...
    def get_health_history(
        self, check_name: Optional[str] = None, time_range: Optional[timedelta] = None
    ) -> List[HealthCheck]:
