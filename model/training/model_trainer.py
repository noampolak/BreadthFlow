...
class ModelTrainer:
    ...
    def __init__(
        ...
        hyperparameter_optimizer: SimpleHyperparameterOptimizer = None
    ):
        """
    ...
    def train_model(
        ...
        try:
            ...
            with self.experiment_manager.start_run(experiment_name, run_name) as run:
                ...
                # Log model
                model_uri = self.experiment_manager.log_model(
                    model=model,
                    model_name=model_name,
                    model_type="sklearn"
                )
    ...
    def optimize_and_train(
        ...
        try:
            ...
            if optimization_method == "sklearn":
                ...
                optimization_result = self.hyperparameter_optimizer.optimize_xgboost(
                    X=X_train,
                    y=y_train
                )
            elif optimization_method == "lightgbm":
                optimization_result = self.hyperparameter_optimizer.optimize_lightgbm(
                    X=X_train,
                    y=y_train
                )
            else:
