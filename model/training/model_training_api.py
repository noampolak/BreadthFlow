...
app = FastAPI(
    title="Model Training API", description="API for model training and experiment tracking in ML pipeline", version="1.0.0"
)
...
async def startup_event():
    ...
    try:
        ...
        spark_session = (
            SparkSession.builder.appName("BreadthFlowModelTraining")
...
async def root():
    """Root endpoint."""
    return {"service": "Model Training API", "version": "1.0.0", "status": "running"}
...
async def train_models(request: TrainingRequest):
    ...
    try:
        ...
        data_prep = model_trainer.prepare_data(
            X=df.drop(columns=[request.target_column]), y=df[request.target_column], test_size=request.test_size
        )
...
async def optimize_and_train(request: OptimizationRequest):
    ...
    try:
        ...
        # Prepare data
        data_prep = model_trainer.prepare_data(X=df.drop(columns=[request.target_column]), y=df[request.target_column])
...
async def list_experiments():
    ...
    try:
        ...
        return {"experiments": experiments, "timestamp": datetime.now().isoformat()}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_best_model(experiment_name: str, metric: str = "test_accuracy"):
    ...
    try:
        ...
        return {"best_model": best_model, "timestamp": datetime.now().isoformat()}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
