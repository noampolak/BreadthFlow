...
class TechnicalAnalysisStrategy(BaseSignalStrategy):
    ...
    def generate_signals(self, data: Dict[str, Any], config: SignalConfig):
        ...
        try:
            ...
            signals = self.technical_indicators.generate_technical_signals(stock_data, indicators, self.config)
    ...
    def _generate_trading_signals(self, data: DataFrame) -> DataFrame:
        ...
        if "rsi" in signals.columns:
            ...
            signals["rsi_neutral"] = ((signals["rsi"] >= rsi_oversold) & (signals["rsi"] <= rsi_overbought)).astype(int)
        ...
        if "macd_line" in signals.columns and "macd_signal" in signals.columns:
            ...
            signals["macd_crossover"] = (
                ...
                & (signals["macd_line"].shift(1) <= signals["macd_signal"].shift(1))
            ).astype(int)
        ...
        if "bb_upper" in signals.columns and "bb_lower" in signals.columns:
            ...
            signals["bb_squeeze"] = ((signals["bb_upper"] - signals["bb_lower"]) / signals["bb_middle"] < 0.1).astype(int)
    ...
    def _calculate_signal_metrics(self, signals: DataFrame) -> DataFrame:
        ...
        signal_strength = np.where(total_signals > 0, (buy_strength - sell_strength) / total_signals, 0)
        ...
        signals["signal_type"] = np.where(signal_strength > 0.3, "buy", np.where(signal_strength < -0.3, "sell", "hold"))
    ...
    def get_strategy_signals(self, data: DataFrame) -> Dict[str, Series]:
        ...
        if "rsi" in data.columns:
            ...
            signals["rsi_strategy"] = np.where(data["rsi"] < rsi_oversold, 1, np.where(data["rsi"] > rsi_overbought, -1, 0))
        ...
        if "bb_upper" in data.columns and "bb_lower" in data.columns:
            signals["bb_strategy"] = np.where(
                data["close"] < data["bb_lower"], 1, np.where(data["close"] > data["bb_upper"], -1, 0)
            )
