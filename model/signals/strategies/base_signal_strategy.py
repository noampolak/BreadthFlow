...
class BaseSignalStrategy(ABC):
    ...
    def validate_config(self, config: SignalConfig) -> bool:
        ...
        if (
            hasattr(config, "required_resources")
            and config.required_resources
            and isinstance(config.required_resources, (list, tuple))
        ):
            for required_resource in self.required_data:
        ...
        if (
            hasattr(config, "required_timeframes")
            and config.required_timeframes
            and isinstance(config.required_timeframes, (list, tuple))
        ):
    ...
    def preprocess_data(self, data: Dict[str, Any], config: SignalConfig):
        ...
        for resource_name, resource_data in data.items():
            ...
            if len(resource_data) < min_points:
                logger.warning(f"Insufficient data points for {resource_name}: {len(resource_data)} < {min_points}")
                continue
