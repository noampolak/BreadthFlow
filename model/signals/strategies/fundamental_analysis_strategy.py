...
class FundamentalAnalysisStrategy(BaseSignalStrategy):
    ...
    def generate_signals(self, data: Dict[str, pd.DataFrame], 
        ...
        try:
            ...
            signals = self.fundamental_indicators.generate_fundamental_signals(
                signals, indicators, self.config
            )
    ...
    def _merge_fundamental_data(self, stock_data: pd.DataFrame, 
                               processed_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        if "revenue" in processed_data:
            ...
            if not revenue_data.empty and 'date' in revenue_data.columns:
                ...
                revenue_data = revenue_data.set_index('date').reindex(
                    signals['date'], method='ffill'
                ).reset_index()
        ...
        if "market_cap" in processed_data:
            ...
            if not market_cap_data.empty and 'date' in market_cap_data.columns:
                ...
                market_cap_data = market_cap_data.set_index('date').reindex(
                    signals['date'], method='ffill'
                ).reset_index()
    ...
    def _generate_valuation_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        signals = self.fundamental_indicators.get_valuation_signals(
            signals, valuation_indicators, thresholds
        )
    ...
    def _generate_growth_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        signals['growth_neutral'] = ((signals['growth_score'] >= -growth_threshold) & 
                                   (signals['growth_score'] <= growth_threshold)).astype(int)
    ...
    def _generate_profitability_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        signals['profitability_neutral'] = ((signals['profitability_score'] >= 0.05) & 
                                          (signals['profitability_score'] <= profitability_threshold)).astype(int)
    ...
    def _calculate_signal_metrics(self, signals: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'fundamental_score' in signals.columns:
            ...
            signal_strength = 2 * (fundamental_score - fundamental_score.min()) / (fundamental_score.max() - fundamental_score.min()) - 1
        ...
        signals['signal_type'] = np.where(signals['signal_strength'] > 0.3, 'buy',
                                        np.where(signals['signal_strength'] < -0.3, 'sell', 'hold'))
    ...
    def get_strategy_signals(self, data: pd.DataFrame) -> Dict[str, pd.Series]:
        ...
        if 'valuation_score' in data.columns:
            signals['valuation_strategy'] = np.where(data['valuation_score'] > 0.7, 1,
                                                   np.where(data['valuation_score'] < 0.3, -1, 0))
        
        # Growth strategy
        if 'growth_score' in data.columns:
            signals['growth_strategy'] = np.where(data['growth_score'] > 0.1, 1,
                                                np.where(data['growth_score'] < -0.1, -1, 0))
        
        # Profitability strategy
        if 'profitability_score' in data.columns:
            signals['profitability_strategy'] = np.where(data['profitability_score'] > 0.15, 1,
                                                       np.where(data['profitability_score'] < 0.05, -1, 0))
        
        # Composite fundamental strategy
        if 'fundamental_score' in data.columns:
            signals['fundamental_strategy'] = np.where(data['fundamental_score'] > 0.6, 1,
                                                     np.where(data['fundamental_score'] < 0.4, -1, 0))
    ...
    def get_signal_summary(self, signals: pd.DataFrame) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'profitability_score_avg': signals['profitability_score'].mean() if 'profitability_score' in signals.columns else 0.0
        }
