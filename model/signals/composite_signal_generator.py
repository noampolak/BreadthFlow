...
class CompositeSignalGenerator(SignalGeneratorInterface):
    ...
    def generate_signals(self, config: SignalConfig, data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        try:
            ...
            # Log success
            logger.info(f"Composite signal generation successful: {len(strategy_results)} strategies, {len(composite_signals)} signals")
    ...
    def _generate_strategy_signals(self, config: SignalConfig, 
    ...
    def _combine_strategy_signals(self, strategy_results: Dict[str, pd.DataFrame], 
    ...
    def _weighted_average_combination(self, strategy_results: Dict[str, pd.DataFrame], 
                                    config: SignalConfig) -> pd.DataFrame:
    ...
    def _voting_combination(self, strategy_results: Dict[str, pd.DataFrame], 
                          config: SignalConfig) -> pd.DataFrame:
        ...
        composite_signals['signal_type'] = np.where(
            buy_votes >= min_votes, 'buy',
            np.where(sell_votes >= min_votes, 'sell', 'hold')
        )
    ...
    def _ensemble_combination(self, strategy_results: Dict[str, pd.DataFrame], 
                            config: SignalConfig) -> pd.DataFrame:
    ...
    def _apply_consensus_filtering(self, composite_signals: pd.DataFrame, 
    ...
    def _calculate_final_metrics(self, composite_signals: pd.DataFrame, 
                               strategy_results: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        if 'signal_strength' in composite_signals.columns and 'confidence' in composite_signals.columns:
            composite_signals['signal_quality'] = (
                abs(composite_signals['signal_strength']) * composite_signals['confidence']
            )
