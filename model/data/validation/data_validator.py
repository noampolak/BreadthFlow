...
class DataValidator:
    ...
    def _validate_completeness(self, data: Any, context: Dict[str, Any]) -> ValidationResult:
        ...
        if data is None:
            return ValidationResult(
                is_valid=False, severity=ValidationSeverity.ERROR, message="Data is None", details={"missing": "all_data"}
            )

        if isinstance(data, (list, tuple)) and len(data) == 0:
            return ValidationResult(
                is_valid=False, severity=ValidationSeverity.WARNING, message="Data is empty", details={"count": 0}
            )

        if isinstance(data, dict) and len(data) == 0:
            return ValidationResult(
                is_valid=False, severity=ValidationSeverity.WARNING, message="Data dictionary is empty", details={"keys": 0}
            )
    ...
    def _validate_quality(self, data: Any, context: Dict[str, Any]) -> ValidationResult:
        ...
        if data is None:
            return ValidationResult(
                is_valid=False, severity=ValidationSeverity.ERROR, message="Data is None", details={"quality_score": 0}
            )
    ...
    def _validate_consistency(self, data: Any, context: Dict[str, Any]) -> ValidationResult:
        ...
        if data is None:
            return ValidationResult(
                is_valid=False, severity=ValidationSeverity.ERROR, message="Data is None", details={"consistency": "none"}
            )
    ...
    def get_summary(self, results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_valid": all(
                r.is_valid for r in results if r.severity in [ValidationSeverity.ERROR, ValidationSeverity.CRITICAL]
            ),
        }
