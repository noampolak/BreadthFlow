...
class TrainingService:
    ...
    async def _train_model(self, training_id: str, request: TrainingRequest):
        ...
        try:
            ...
            training_session = self.db.query(TrainingSession).filter(TrainingSession.training_id == training_id).first()
    ...
    def _estimate_training_duration(self, request: TrainingRequest) -> int:
        ...
        base_duration = {"random_forest": 5, "xgboost": 8, "neural_network": 15, "svm": 10, "logistic_regression": 3}
        ...
        timeframe_multiplier = {"1min": 3, "5min": 2, "15min": 1.5, "1hour": 1.2, "1day": 1}
    ...
    def get_training_history(self, limit: int = 20) -> List[TrainingHistory]:
        ...
        training_sessions = self.db.query(TrainingSession).order_by(desc(TrainingSession.start_time)).limit(limit).all()
    ...
    def get_models(self) -> List[ModelInfo]:
        ...
        models = self.db.query(TrainedModel).order_by(desc(TrainedModel.created_at)).all()
    ...
    async def delete_model(self, model_id: str):
        ...
        model = self.db.query(TrainedModel).filter(TrainedModel.model_id == model_id).first()
