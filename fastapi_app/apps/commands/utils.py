...
class CommandService:
    ...
    async def execute_command(self, request: CommandRequest, background_tasks) -> CommandResponse:
        ...
        command_execution = CommandExecution(
            command_id=command_id, command=request.command, status=CommandStatus.PENDING, start_time=start_time
        )
    ...
    async def _execute_sync_command(self, command_id: str, command: str) -> CommandResponse:
        ...
        try:
            ...
            command_execution = self.db.query(CommandExecution).filter(CommandExecution.command_id == command_id).first()
            ...
            # Execute the command
            result = subprocess.run(command.split(), capture_output=True, text=True, timeout=300)  # 5 minute timeout
    ...
    async def _execute_background_command(self, command_id: str, command: str):
        ...
        try:
            ...
            result = subprocess.run(
                command.split(), capture_output=True, text=True, timeout=1800  # 30 minute timeout for background tasks
            )
    ...
    def get_command_history(self, limit: int = 50) -> List[CommandHistory]:
        ...
        command_executions = self.db.query(CommandExecution).order_by(desc(CommandExecution.start_time)).limit(limit).all()
