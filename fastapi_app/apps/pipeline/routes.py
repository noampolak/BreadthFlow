...
async def get_pipeline_runs(skip: int = 0, limit: int = 10, status: Optional[str] = None, db: Session = Depends(get_db)):
...
async def start_pipeline(config: PipelineConfig, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    if running_pipeline:
        raise HTTPException(status_code=400, detail=f"Pipeline {running_pipeline.run_id} is already running")
...
async def stop_pipeline(run_id: str, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
