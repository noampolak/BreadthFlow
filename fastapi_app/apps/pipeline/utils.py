...
class PipelineService:
    ...
    async def get_runs(self, skip: int = 0, limit: int = 10, status: Optional[str] = None) -> List[PipelineRun]:
    ...
    async def get_running_pipeline(self) -> Optional[PipelineRun]:
        ...
        return self.db.query(PipelineRun).filter(PipelineRun.status == PipelineStatus.RUNNING).first()

    async def get_status(self) -> PipelineStatusResponse:
        """Get pipeline status and statistics"""
        total_runs = self.db.query(PipelineRun).count()
        successful_runs = self.db.query(PipelineRun).filter(PipelineRun.status == PipelineStatus.COMPLETED).count()
        failed_runs = self.db.query(PipelineRun).filter(PipelineRun.status == PipelineStatus.FAILED).count()
        running_runs = self.db.query(PipelineRun).filter(PipelineRun.status == PipelineStatus.RUNNING).count()
        stopped_runs = self.db.query(PipelineRun).filter(PipelineRun.status == PipelineStatus.STOPPED).count()

        avg_duration = self.db.query(func.avg(PipelineRun.duration)).filter(PipelineRun.duration.isnot(None)).scalar() or 0
