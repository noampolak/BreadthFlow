    ...
    try:
        ...
        if pipeline_run:
            ...
            if websocket_manager:
                await websocket_manager.broadcast_pipeline_update(pipeline_run.run_id, "running")

        # Execute pipeline via Spark command server
        async with httpx.AsyncClient() as client:
            response = await client.post(
                "http://spark-master:8081/execute",
                json={"command": "spark_streaming_start_demo", "parameters": config},
                timeout=300.0,
            )
            ...
            if response.status_code == 200:
                ...
                # Broadcast update via WebSocket
                if websocket_manager:
                    await websocket_manager.broadcast_pipeline_update(pipeline_run.run_id, "completed")
            else:
                ...
                # Broadcast update via WebSocket
                if websocket_manager:
                    await websocket_manager.broadcast_pipeline_update(pipeline_run.run_id, "failed")
...
async def stop_pipeline_task(run_id: str):
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                "http://spark-master:8081/execute",
                json={"command": "spark_streaming_stop", "parameters": {"run_id": run_id}},
                timeout=30.0,
            )
            ...
            if pipeline_run:
                ...
                # Broadcast update via WebSocket
                if websocket_manager:
                    await websocket_manager.broadcast_pipeline_update(pipeline_run.run_id, "stopped")
